...
function TransactionHistoryItem({
  ...
  return (
    ...
    >
      ...
      >
        <Icon className="w-3.5 h-3.5" style={{ color: iconStyle.color }} />
      </div>
    ...
    <>
      ...
      <aside
        className={cn(
          ...
          'fixed right-0 top-0 w-full max-w-[280px] 2xl:relative 2xl:w-[240px] 2xl:max-w-none',
          'max-md:bottom-0 max-md:top-auto max-md:max-h-[85vh] max-md:rounded-t-2xl',
          isWalletOpen
            ? 'translate-x-0 max-md:translate-y-0 flex'
            : 'translate-x-full max-md:translate-y-full max-2xl:hidden',
          '2xl:flex 2xl:translate-x-0 2xl:translate-y-0'
        )}
      >
        <div className="flex flex-col flex-1 min-h-0 py-5 px-4">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-3)] mb-3">
            Connected Wallet
          </p>
          ...
          {!isLoggedIn ? (
            <div className="space-y-2">
              ...
              <button type="button" onClick={() => setIsGoogleModalOpen(true)} className={btnGhost}>
                Sign in with Google
              </button>
            </div>
          ) : (
            <>
              <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-3.5 mb-4">
                <div className="flex items-center gap-2 mb-2">
                  <Wallet2 className="w-3.5 h-3.5 text-[var(--text-3)]" />
                  <span className="text-xs text-[var(--text-2)]">Web3 Wallet</span>
                </div>
              ...
              <div className="mb-5">
                <p className="text-[11px] text-[var(--text-3)] mb-1">ETH Balance</p>
                <p className="text-[22px] font-semibold text-[var(--text-1)] tabular-nums">
                  {balancesLoading ? (
                ...
                <p className="text-xs text-[var(--text-2)] tabular-nums mt-0.5">
                  {balancesLoading ? '—' : formatCurrency(usdTotal)}
                </p>
                {balancesRefreshing && !balancesLoading ? (
                  <p className="text-[10px] text-[var(--text-3)] mt-1">Updating…</p>
                ) : null}
              </div>
              ...
              <div className="mb-5">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-[10px] text-[var(--text-3)]">{wallet.nfts.length} NFT</span>
                </div>
                {wallet.nfts.length === 0 ? (
                  <div className="flex flex-col items-center py-3 text-center">
                    <ImageOff className="w-[18px] h-[18px] text-[var(--text-3)] mb-1.5" />
                    <p className="text-xs text-[var(--text-3)]">No NFTs yet</p>
                  </div>
                ) : (
              ...
              <div className="flex-1 min-h-0 flex flex-col">
                ...
                <div className="overflow-y-auto min-h-0 flex-1">
                  {recentTx.length === 0 ? (
                    <p className="text-xs text-[var(--text-3)] py-4 text-center">No transactions yet</p>
                  ) : (
                    recentTx.map((tx) => (
        ...
        {isLoggedIn ? (
          <div className="shrink-0 px-4 pb-5 pt-3 border-t border-[var(--border-subtle)]">
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-[var(--text-3)] hover:text-[var(--danger)] transition-colors duration-150"
            >
